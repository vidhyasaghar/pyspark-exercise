...
def transform_it_data(df1: DataFrame, df2: DataFrame, df3: DataFrame) -> DataFrame:
    ...
    :rtype: DataFrame
    """
    result = df1.join(df2, on="id", how="inner").filter(F.col("area") == "IT").orderBy(F.col("sales_amount").desc()).limit(100)
    return result
...
def transform_marketing_address_info(df1: DataFrame, df2: DataFrame, df3: DataFrame) -> DataFrame:
    ...
    result = (
        ...
        .withColumn("address_without_zip", F.regexp_replace(F.col("address"), r"\d{4} [A-Z]{2}", ""))
        .withColumn("only_area", F.regexp_replace(F.col("address_without_zip"), r"([A-Za-z ]+ \d+)", ""))
...
def transform_department_breakdown(df1: DataFrame, df2: DataFrame, df3: DataFrame) -> DataFrame:
    ...
    result = (
        ...
        .agg(
            ...
            (F.sum(F.col("calls_successful")) / F.sum(F.col("calls_made")) * 100).alias("success_rate"),
        )
...
@transform("top_3")
def transform_best_performer_per_department(df1: DataFrame, df2: DataFrame, df3: DataFrame) -> DataFrame:
    # pylint: disable=unused-argument
    ...
    result = (
        ...
        .withColumn(
            "rank",
            F.rank().over(Window.partitionBy("area").orderBy(F.col("sales_amount").desc(), F.col("success_rate").desc())),
        )
        .filter(F.col("rank") <= 3)
        .select(
            "area",
...
def transform_best_salesperson_per_country(df1: DataFrame, df2: DataFrame, df3: DataFrame) -> DataFrame:
    # pylint: disable=unused-argument
    ...
    result = (
        ...
        .withColumn(
            "rank",
            F.rank().over(Window.partitionBy("country").orderBy(F.col("total_quantity").desc(), F.col("sales_amount").desc())),
        )
        .filter(F.col("rank") == 1)
        .select(
            "name",
